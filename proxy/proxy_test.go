...
func TestNewMiddleware_zeroConfig(t *testing.T) {
	cfg := &config.Backend{
		ExtraConfig: map[string]interface{}{
			cbreaker.Namespace: map[interface{}]interface{}{
	...
	data := cbreaker.ConfigGetter(cfg.ExtraConfig).(cbreaker.Config)
...
func TestNewMiddleware_Config(t *testing.T) {
	for _, cfg := range []*config.Backend{
		{},
		{ExtraConfig: map[string]interface{}{cbreaker.Namespace: 42}},
	} {
		resp := proxy.Response{}
